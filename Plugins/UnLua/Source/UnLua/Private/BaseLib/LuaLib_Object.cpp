...
static int32 FSoftObjectPtr_LoadSynchronous(lua_State* L)
{
    ...
    FSoftObjectPtr* SoftObjectPtr = static_cast<FSoftObjectPtr*>(GetCppInstanceFast(L, 1));
    if (!SoftObjectPtr)
        return luaL_error(L, "invalid FSoftObjectPtr");

    UnLua::PushUObject(L, SoftObjectPtr->LoadSynchronous());
    return 1;
}
